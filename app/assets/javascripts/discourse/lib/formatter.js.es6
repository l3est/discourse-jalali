...
// topic list long date
export function longDate(dt) {
  if (!dt) return;
  //return moment(dt).format(I18n.t("dates.long_with_year"));
  //return moment(dt, "MMM D, YYYY h:mm a").format("jYYYY/jMM/jD"); // Parse a Jalaali date
//return moment(dt).jYear().toString() + moment(dt).jMonth().toString();
return moment(dt).format("jD jMMMM jYYYY HH:mm");

  //return m.format('jYYYY/jM/jD [is] YYYY/M/D').format("jYYYY/jMM/jD")
...
export function longDateNoYear(dt) {
  ...
  if ((new Date()).getFullYear() !== dt.getFullYear()) {
    ...
    return moment(dt).format("jYY/jM/jD");
  } else {
    return moment(dt).format("jM/jD");
  }
}
